		...
		$("#pop_add_cust_submit").click(function () {
			...
			if(fn.length == 0){
				...
				var addNewCustomer = $.ajax({
					url		: '<?=base_url()?>index.php/pos/addNewCustomer?fn='+fn+'&em='+em+'&mb='+mb,
					type	: 'GET',
					cache	: false,
					data	: {
						format: 'json'
		...
		$("#pcodeBarcode").keyup(function (e) {
			if (e.keyCode == 13) {
				...
				var resultPrice = $.ajax({
							url		: '<?=base_url()?>index.php/pos/getProductDetail?pcode='+pcode+'&outlet_id='+temp_outlet,
							type	: 'GET',
							cache	: false,
							data	: {
								format: 'json'
...
<script type="text/javascript">
	$(document).ready(function(){
		$('#holdmodel').on('shown.bs.modal', function () {
			...
			var addNewCustomer = $.ajax({
					url		: '<?=base_url()?>index.php/pos/loadCustomer',
					type	: 'GET',
					cache	: false,
					data	: {
						format: 'json'
		...
		$('#totalSales').on('shown.bs.modal', function () {	
			...
			var loadTodaySales = $.ajax({
						url		: '<?=base_url()?>index.php/pos/loadTodaySales?outlet_id='+temp_outlet,
						type	: 'GET',
						cache	: false,
						data	: {
							format: 'json'
	...
	function searchOpenedBill(ele){
		...
		if(ele.length == 0){
			...
			xmlhttp.open("GET","<?=base_url()?>index.php/pos/getopenedBill?q=",true);
			xmlhttp.send();
		...
		xmlhttp.open("GET","<?=base_url()?>index.php/pos/getopenedBill?q="+ele,true);
		xmlhttp.send();
	}
					...
					<div class="row" style="padding-left: 10px; padding-right: 10px; overflow: scroll; height: 400px; margin-top: 10px;" id="searchOpenedResult">
						...
						<?php
                            ...
                            for ($op = 0; $op < count($openedBillData); ++$op) {
								...
								<a href="<?=base_url()?>index.php/pos?suspend_id=<?php echo $openedBill_id; ?>" style="text-decoration: none;">
